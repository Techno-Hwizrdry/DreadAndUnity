...
public class LevelGenerator : MonoBehaviour {
	...
	public int floorHoles;  // Maximum holes generated in the floor.
	...
	private Text txtLevelCounter;  // A reference to the "Text_Level_Counter" text in Canvas.
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				if (holesToMake > 0 && Random.value < 0.1) {
					holesToMake--;
				} else {
	...
	// This method will update the level counter displayed in the user's HUD.
	private void UpdateLevelCounter() {
		level++;
        txtLevelCounter = GameObject.Find("Text_Level_Counter").GetComponent<Text>();
		txtLevelCounter.text = level.ToString ();
    }
}
